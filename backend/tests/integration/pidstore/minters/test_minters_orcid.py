...
def test_minter_orcid(app, create_record_factory):
...
def test_minter_orcid_empty(app, create_record_factory):
...
def test_minter_orcid_already_existing(app, create_record_factory):
    orcid_value = faker.orcid()
...
def test_minter_orcid_missing_schema(app, create_record_factory):
    orcid_value = faker.orcid()
    data = {"ids": [{"value": orcid_value, "schema": "ORCID"}]}
    record = create_record_factory("aut", data=data)
