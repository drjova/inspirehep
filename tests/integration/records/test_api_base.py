...
def test_get_records_from_pids(base_app, db, create_record):
    records = [create_record("lit"), create_record("lit"), create_record("lit")]
    ...
    result = InspireRecord.get_records_from_pids(pids)
    result = list(result)
...
def test_get_records_from_pids_with_not_existing_pids(base_app, db, create_record):
    pids = [("lit", "123"), ("aut", "234"), ("lit", "345")]

    expected_result_len = 0

    result_uuids = InspireRecord.get_records_from_pids(pids)
    result_uuids = list(result_uuids)

    assert expected_result_len == len(result_uuids)


def test_get_records_from_pids_with_empty(base_app, db, create_record):
    pids = []
